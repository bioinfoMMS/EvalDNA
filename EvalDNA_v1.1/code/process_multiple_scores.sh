...
while :; do
    case $1 in
        -h|-\?|--help|'')
            echo 'Usage: bash process_multiple_scores.sh output_directory_name'
            echo
            echo 'Example:  bash process_multiple_scores.sh EvalDNA_results'
            exit
	    ;;
	*)
	    break 
...
if [ -d "$1" ]; then
	...
	echo "Error: can not find the given directory name."
fi
